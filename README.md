...
**Pre-release.** `main` is the native Rust application. Physical desktop acceptance and public signing are still pending; adopting the native development baseline did not publish a release.
...
[One CI workflow](https://github.com/d-sandhu/screenfling/actions/workflows/check.yml) builds and packages Windows x86-64, macOS Apple Silicon, and Linux x86-64. A small Rust suite checks the failure-prone logic, and RustSec advisories are enforced. Windows additionally captures and crops a synthetic window. Native clipboard checks compare every pixel across processes on Windows/macOS. Linux checks exercise X11 window-state recovery, headless Wayland with real portal/PipeWire services, and exact Stage/Reveal routing in a disposable WezTerm instance.
