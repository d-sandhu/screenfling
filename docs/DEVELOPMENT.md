...
The script packages an existing release executable. Windows receives a portable ZIP. macOS receives an ad-hoc-signed `.app` in a tar archive and runs `codesign --verify --strict`. Linux receives the binary, desktop launcher, and icon in a tar archive. Each package includes an offline usage README, the project's license, and third-party notices collected from the resolved Cargo packages, including nested native-library/font license texts. The inventory may include build-time dependencies; it is not a claim that every listed crate is linked into the final executable.
