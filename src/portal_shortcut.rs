...
impl PortalShortcut {
    pub fn start() -> Self {
        ...
        if worker.is_none() {
            desktop::post(Message::ShortcutStatus(
                "Could not start the shortcut portal. The Capture button remains available.".into(),
            ));
        }
        Self { stop, worker }
    }
...
async fn cancellable<T>(
    ...
    race(
        async {
            future
                .await
                .map_err(|_| "Shortcut portal unavailable.".into())
        },
        async {
            let _ = stop.recv().await;
            Err("Shortcut request stopped.".into())
        },
    )
    .await
}
async fn run(stop: &async_channel::Receiver<()>) -> Result<(), String> {
    ...
    let result = async {
        ...
        let handle = serde_json::to_value(&session)
            .map_err(|_| "Could not identify the shortcut session.")?;
        let handle = handle
            .as_str()
            .ok_or("Invalid shortcut session identifier.")?;
        let keys = [
            NewShortcut::new("capture", "Capture a region with ScreenFling")
                .preferred_trigger("CTRL+SHIFT+9"),
        ];
        let mut activated = Box::pin(cancellable(proxy.receive_activated(), stop).await?);
        let binding = cancellable(
            proxy.bind_shortcuts(&session, &keys, None, Default::default()),
            stop,
        )
        .await?
        .response()
        .map_err(|_| "Shortcut permission was not granted.")?;
        let key = binding
            .shortcuts()
            .iter()
            .find(|key| key.id() == "capture")
            .ok_or("The desktop did not bind Capture.")?;
        desktop::post(Message::ShortcutStatus(format!(
            "Capture shortcut: {}",
        ...
        let mut closed = Box::pin(
            session
                .receive_closed()
                .await
                .map_err(|_| "The shortcut session is unavailable.")?,
        );
        race(
            async {
                loop {
                    let event = activated
                        .next()
                        .await
                        .ok_or("The shortcut portal disconnected.")?;
            ...
            race(
                ...
                async {
                    ...
                    Err("The shortcut session was closed.".into())
                },
            ),
        )
        .await
