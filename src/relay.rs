...
pub fn connect(path: &std::path::Path) -> Result<Socket> {
    let socket = Socket::new(Domain::UNIX, Type::STREAM, None)
        .map_err(|_| "Local AF_UNIX sockets are unavailable on this desktop.")?;
    let address =
        SockAddr::unix(path).map_err(|_| "The local socket path is too long or invalid.")?;
    socket
        .connect_timeout(&address, Duration::from_secs(2))
        .map_err(
            |_| "The selected local terminal is unavailable or did not accept a connection.",
        )?;
    Ok(socket)
}
...
pub fn command(
    ...
    listener
        .bind(&SockAddr::unix(&endpoint).map_err(io_error)?)
        .map_err(io_error)?;
    #[cfg(unix)]
    {
    ...
    process
        ...
        .args(arguments)
        .env("WEZTERM_UNIX_SOCKET", &endpoint)
    ...
    let mut stdin = child
        ...
        .stdin
        .take()
        .ok_or("The WezTerm input pipe is unavailable.")?;
    let stdout = child
        .0
        .stdout
        .take()
        .ok_or("The WezTerm output pipe is unavailable.")?;
    let (input_sender, input_result) = mpsc::sync_channel(1);
    let writer = thread::Builder::new()
        .name("screenfling-cli-input".into())
        .spawn(move || {
    ...
    let reader = thread::Builder::new()
        .name("screenfling-cli-output".into())
        .spawn(move || {
            let mut bytes = Vec::new();
            let result = stdout.take((MAX_OUTPUT + 1) as u64).read_to_end(&mut bytes);
            let result: Result<Vec<u8>> = if result.is_err() || bytes.len() > MAX_OUTPUT {
                Err("The terminal returned an invalid or excessive response.".into())
            } else {
                Ok(bytes)
            };
    ...
    let result = (|| {
        ...
        loop {
            if Instant::now() >= deadline {
                return Err("The terminal operation timed out. Its result is uncertain; inspect the selected pane before doing anything else. It will not be retried.".into());
            }
...
fn io_error(_: impl std::fmt::Display) -> String {
    "The private local terminal transport is unavailable. No fallback destination was used.".into()
}
...
mod tests {
    ...
    fn replaced_clipboard_blocks_the_write() {
        ...
        assert!(
            guarded_write(&mut output, b"stage", &mut || Err(
                "replaced clipboard".into()
            ))
            .is_err()
        );
        assert!(output.is_empty());
        assert_eq!(
            guarded_write(&mut output, b"stage", &mut || Ok(())).unwrap(),
            5
        );
