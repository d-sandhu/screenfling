...
fn current_user() -> Result<String> {
    ...
    if unsafe { OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &mut token) } == 0 { return Err("Could not inspect the current Windows user.".into()); }
    let result = (|| {
        let mut length = 0;
        unsafe { GetTokenInformation(token, TokenUser, ptr::null_mut(), 0, &mut length); }
        if length < size_of::<TOKEN_USER>() as u32 || length > 16384 { return Err("Invalid Windows user token.".into()); }
        let mut buffer = vec![0usize; (length as usize).div_ceil(size_of::<usize>())];
        if unsafe { GetTokenInformation(token, TokenUser, buffer.as_mut_ptr().cast(), length, &mut length) } == 0 { return Err("Could not read the Windows user token.".into()); }
...
fn privileged(sid: &str, user: &str) -> bool {
    sid == user || matches!(sid, "S-1-5-18" | "S-1-5-32-544" | "S-1-5-80-956008885-3418522649-1831038044-1853292631-2271478464")
}
...
pub fn validate(path: &Path, require_user_owner: bool, socket: bool) -> Result<Vec<u64>> {
    ...
    let code = unsafe { GetNamedSecurityInfoW(name.as_ptr(), SE_FILE_OBJECT, OWNER_SECURITY_INFORMATION | DACL_SECURITY_INFORMATION, &mut owner, ptr::null_mut(), &mut dacl, ptr::null_mut(), &mut descriptor) };
    if code != 0 { return Err("Could not inspect the configured path's Windows permissions.".into()); }
    ...
    unsafe {
        for index in 0..(*dacl).AceCount {
            ...
            if GetAce(dacl, index as u32, &mut ace) == 0 { return Err("Could not inspect a Windows access rule.".into()); }
    ...
    let handle = unsafe { CreateFileW(name.as_ptr(), FILE_READ_ATTRIBUTES, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, ptr::null(), OPEN_EXISTING, FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OPEN_REPARSE_POINT, ptr::null_mut()) };
    if handle == INVALID_HANDLE_VALUE { return Err("Could not read the configured Windows file identity.".into()); }
    let result = (|| {
        let mut info = unsafe { std::mem::zeroed::<BY_HANDLE_FILE_INFORMATION>() };
        if unsafe { GetFileInformationByHandle(handle, &mut info) } == 0 { return Err("Could not identify the configured Windows file.".into()); }
        if info.dwFileAttributes & FILE_ATTRIBUTE_REPARSE_POINT != 0 {
            let mut tag = unsafe { std::mem::zeroed::<FILE_ATTRIBUTE_TAG_INFO>() };
            if unsafe { GetFileInformationByHandleEx(handle, FileAttributeTagInfo, (&mut tag as *mut FILE_ATTRIBUTE_TAG_INFO).cast(), size_of::<FILE_ATTRIBUTE_TAG_INFO>() as u32) } == 0 || !socket || tag.ReparseTag != 0x8000_0023 {
                return Err("The configured path is an unexpected Windows reparse point.".into());
            }
        } else if socket { return Err("The selected endpoint is not a Windows AF_UNIX socket.".into()); }
        let mut result = vec![info.dwVolumeSerialNumber as u64, (info.nFileIndexHigh as u64) << 32 | info.nFileIndexLow as u64, info.dwFileAttributes as u64];
        if info.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY == 0 {
            result.extend([(info.ftCreationTime.dwHighDateTime as u64) << 32 | info.ftCreationTime.dwLowDateTime as u64, (info.ftLastWriteTime.dwHighDateTime as u64) << 32 | info.ftLastWriteTime.dwLowDateTime as u64, (info.nFileSizeHigh as u64) << 32 | info.nFileSizeLow as u64]);
        }
        acl_hash.update(owner.as_bytes());
        let hash = acl_hash.finalize();
        for part in hash.chunks_exact(8) { result.push(u64::from_le_bytes(part.try_into().map_err(|_| "Invalid permission fingerprint.")?)); }
        Ok(result)
    })();
