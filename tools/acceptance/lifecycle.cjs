...
const watchdog = setTimeout(() => {
  ...
  process.stderr.write('{"acceptance":"packaged-lifecycle","status":"failed","reason":"runner-timeout"}\n');
  process.exit(1);
}, 60_000);

main()
  .catch(() => {
    process.stderr.write('{"acceptance":"packaged-lifecycle","status":"failed","reason":"lifecycle-check-failed"}\n');
    process.exitCode = 1;
  })
